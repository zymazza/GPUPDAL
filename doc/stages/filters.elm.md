...
PDG compatibility mode has a bounded exact CUDA implementation for finite
logical-double XYZ, unsigned-byte Classification, and a whole-view raster of
at most 4,096 cells. It preserves upstream's fractional-cell binning,
source-stable equal-height order (including signed zero), strict threshold
comparison, point order, and diagnostic bytes. The planner admits only a
standalone upload-filter-spill region; unsupported options, nonfinite input,
larger rasters, and adjacent Grid composition retain the exact host path.
Automatic placement remains disabled because the same-machine B0024 complete
pipeline is exact but slower than pinned PDAL. This bounded lane is not a
scalable/tiled Grid or a P3 completion claim.
