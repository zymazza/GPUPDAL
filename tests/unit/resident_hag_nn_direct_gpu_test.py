#!/usr/bin/env python3
"""Exact public-process gate for automatic direct HAG-NN count-one."""
...
def automatic_environment():
    ...
    environment["PDG_REQUIRE_AUTOMATIC_HAG_NN_RESIDENT"] = "1"
    return environment


def write_pipeline(path, source, destination, count=1,
...
def fallback_case(root, name, pdg, oracle, source, default_environment=None,
                  count=1, producer_options=None, writer_options=None,
...
def main():
    if len(sys.argv) != 4 or sys.argv[1] != "--automatic":
        raise SystemExit(
            "usage: resident_hag_nn_direct_gpu_test.py --automatic PDG PDAL")
    pdg = pathlib.Path(sys.argv[2]).resolve()
    oracle = pathlib.Path(sys.argv[3]).resolve()
    configured = os.environ.get("PDG_HAG_NN_COUNT1_LAS_FILE")
    expected_hash = os.environ.get("PDG_HAG_NN_COUNT1_LAS_SHA256")
    cap_configured = os.environ.get("PDG_HAG_NN_COUNT1_CAP_LAS_FILE")
    cap_expected_hash = os.environ.get("PDG_HAG_NN_COUNT1_CAP_LAS_SHA256")
    if not all((configured, expected_hash, cap_configured, cap_expected_hash)):
        print("set the PDG_HAG_NN_COUNT1[_CAP]_LAS_FILE/SHA256 variables",
              file=sys.stderr)
        return SKIP
    ...
    if mem_available_bytes() < 8 * GIB:
        print("HAG-NN automatic gate skipped below 8 GiB MemAvailable",
              file=sys.stderr)
        return SKIP
    gpu = run(["nvidia-smi", "--query-gpu=memory.free",
               "--format=csv,noheader,nounits"])
    if gpu.returncode != 0 or int(gpu.stdout.splitlines()[0]) < 4096:
        print("HAG-NN automatic gate skipped without 4 GiB free VRAM",
              file=sys.stderr)
        return SKIP

    with tempfile.TemporaryDirectory(
            prefix="pdg-hag-nn-count1-automatic-") as temporary:
        root = pathlib.Path(temporary)
        below = prefix_source(root, oracle, source, 400_002)
        ...
        for name, variable in (
                ...
                ("proof", "PDG_TEST_AUTOMATIC_HAG_NN_PROOF_FAILURE")):
            environment = os.environ.copy()
        ...
        fallback_case(root, "count-two", pdg, oracle, floor, count=2)
