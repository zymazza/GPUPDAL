#!/usr/bin/env python3
"""Physical exact gate for the calibrated eigen-family composition."""
...
def main():
    ...
    configured = os.environ.get("PDG_EIGEN_FAMILY_LAS_FILE")
    expected_hash = os.environ.get("PDG_EIGEN_FAMILY_LAS_SHA256")
    if not configured or not expected_hash:
        print("set PDG_EIGEN_FAMILY_LAS_FILE and "
              "PDG_EIGEN_FAMILY_LAS_SHA256 for the physical gate",
              file=sys.stderr)
        return SKIP
    source = pathlib.Path(configured).resolve()
    if not source.is_file() or source.suffix.lower() != ".las":
        raise AssertionError("PDG_EIGEN_FAMILY_LAS_FILE must name a LAS file")
    if sha256(source) != expected_hash.lower():
        raise AssertionError("physical eigen-family source hash mismatch")
    ...
    if mem_available_bytes() < 8 * GIB:
        print("eigen-family gate skipped below 8 GiB MemAvailable",
              file=sys.stderr)
        return SKIP
    gpu = run(["nvidia-smi", "--query-gpu=memory.free",
               "--format=csv,noheader,nounits"])
    if gpu.returncode != 0 or int(gpu.stdout.splitlines()[0]) < 2048:
        print("eigen-family gate skipped without 2 GiB free VRAM",
              file=sys.stderr)
        return SKIP

    with tempfile.TemporaryDirectory(prefix="pdg-eigen-family-") as temporary:
        ...
        environment["PDG_REQUIRE_NEIGHBORHOOD_EIGENSYSTEM_REUSE"] = "13"
        candidate = run([
        ...
        # B0074 replaces the forced-hybrid rows with the selected executor's
        # complete resident-mode ladder, so the diagnostic must now recognize
        # this measured calibration/executor pair.
        assert execution[
        ...
        assert execution["selected_stage_ids"] == [1, 2, 3, 4], execution
        ...
        assert execution["exact_host_repair"]["repaired_rows"] == 0, execution
        assert execution["exact_device_repair"]["repaired_rows"] == 0, (
            execution)
        ...
        def assert_rejected(name, mutate=None, inject_preflight=False):
            ...
            assert ("required automatic eigen-family resident path was not "
        ...
        assert_rejected(
            ...
            lambda stages: stages[4]["value"].reverse())
        assert_rejected(
            "extra-stage",
            lambda stages: stages.insert(
                5, {"type": "filters.ferry",
