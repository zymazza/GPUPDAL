#!/usr/bin/env python3
"""Exact process matrix for filters.normal and shared neighborhood eigen math."""
...
CASES = (
    Case("default", "las/simple.las", {"type": "filters.normal"}),
    Case("knn-twelve", "las/simple.las",
         {"type": "filters.normal", "knn": 12}),
    Case("always-up-false", "las/simple.las",
         {"type": "filters.normal", "knn": 7, "always_up": False}),
    Case("ties-and-duplicates", "pdg/neighborhood-points.csv",
         {"type": "filters.normal", "knn": 2}),
    Case("empty", "las/no-points.las", {"type": "filters.normal"}),
    Case("radius-fallback", "pdg/neighborhood-points.csv",
         {"type": "filters.normal", "radius": 1.1}, hybrid=False),
    Case("viewpoint-fallback", "las/simple.las",
         {"type": "filters.normal",
          "viewpoint": "POINT Z (637012 849028 1000)"}, hybrid=False),
    Case("refine-fallback", "las/simple.las",
         {"type": "filters.normal", "knn": 8, "refine": True},
         hybrid=False),
    Case("where-fallback", "las/simple.las",
         {"type": "filters.normal", "where": "Classification == 2"},
         hybrid=False),
    Case("large-knn-fallback", "las/simple.las",
         {"type": "filters.normal", "knn": 64}, hybrid=False),
    Case("small-knn-fallback", "las/simple.las",
         {"type": "filters.normal", "knn": 1}, hybrid=False),
    Case("knn-radius-error", "las/simple.las",
         {"type": "filters.normal", "knn": 8, "radius": 1.0},
         hybrid=False),
)
...
def main() -> int:
    ...
    with tempfile.TemporaryDirectory(
        prefix="pdg-normal-matrix-", dir=args.work_dir
    ) as temporary:
        generated = Path(temporary)
        for case in CASES:
            fixture = (args.fixture_root / case.fixture).resolve()
            if not fixture.is_file():
                print(f"missing normal fixture: {fixture}", file=sys.stderr)
                return 2
            ...
            command = [
                ...
                "--case", f"normal-matrix-{case.name}",
    ...
    print(f"exact normal process matrix: {len(CASES)} cases")
    return 0
