#!/usr/bin/env python3
"""Exact bounded process matrix for filters.csf."""
...
CASES = (
    Case("supported-baseline", "returns", {
        "type": "filters.csf", "smooth": False, "iterations": 32,
        "resolution": 25.0,
    }, verbose=True),
    Case("iterations-zero", "returns", {
        "type": "filters.csf", "smooth": False, "iterations": 0,
    }),
    Case("one-step-step-1", "returns", {
        "type": "filters.csf", "smooth": False, "iterations": 1,
        "step": 1.0,
    }),
    Case("fractional-resolution", "fractional", {
        "type": "filters.csf", "smooth": False, "iterations": 32,
        "resolution": 12.5,
    }),
    ...
    Case("missing-return-dims", "no-returns", {
        "type": "filters.csf", "smooth": False, "iterations": 32,
    }),
    Case("all-zero-returns", "zero-returns", {
        "type": "filters.csf", "smooth": False, "iterations": 32,
    }),
    Case("only-return-number-native-error", "only-return-number", {
        "type": "filters.csf", "smooth": False, "iterations": 32,
        "returns": [],
    }),
    Case("only-number-of-returns-native-error", "only-number-of-returns", {
        "type": "filters.csf", "smooth": False, "iterations": 32,
        "returns": [],
    }),
    Case("partial-zero-native-error", "partial-zero", {
        "type": "filters.csf", "smooth": False, "iterations": 32,
    }),
    Case("supported-empty", "empty", {
        "type": "filters.csf", "smooth": False, "iterations": 32,
    }),
    Case("default-smooth-true", "returns", {
        "type": "filters.csf", "smooth": True,
    }, hybrid=False),
    Case("ignore-fallback", "returns", {
        "type": "filters.csf", "smooth": True, "ignore": "Classification[7:7]",
    }, hybrid=False),
    Case("where-fallback", "returns", {
        "type": "filters.csf", "smooth": True, "where": "Classification != 7",
    }, hybrid=False),
    Case("debug-true-no-dir", "returns", {
        "type": "filters.csf", "smooth": True, "debug": True,
    }, hybrid=False),
    Case("partial-zero-error", "partial-zero", {
        "type": "filters.csf", "smooth": True,
    }, hybrid=False),
    Case("invalid-returns-error", "returns", {
        "type": "filters.csf", "smooth": True, "returns": "bogus",
    }, hybrid=False),
    Case("equal-classes-error", "returns", {
        "type": "filters.csf", "smooth": True, "ground_class": 4,
        "other_class": 4,
    }, hybrid=False),
    Case("empty-success", "empty", {"type": "filters.csf", "smooth": True},
         hybrid=False),
)
...
def terrain_rows(kind: str) -> list[tuple[float, float, float, int, int, int]]:
    ...
    for x_index in range(16):
        for y_index in range(16):
            ...
            point += 1
    if kind == "fractional":
        return [(x / 1.25, y / 1.15, z, c, rn, nr) for x, y, z, c, rn, nr in rows]
    return rows
...
def write_fixture(path: Path, kind: str) -> None:
    ...
    elif kind == "only-return-number":
        header = "X,Y,Z,Classification,ReturnNumber\n"
    elif kind == "only-number-of-returns":
        header = "X,Y,Z,Classification,NumberOfReturns\n"
    else:
    ...
    for index, (x, y, z, classification, rn, nr) in enumerate(rows):
        ...
        if kind == "no-returns":
            ...
            values = (x, y, z, classification, rn)
        elif kind == "only-number-of-returns":
            values = (x, y, z, classification, nr)
        else:
...
def main() -> int:
    ...
    with tempfile.TemporaryDirectory(prefix="pdg-csf-matrix-",
    ...
    print(f"exact csf process matrix: {executed} cases")
    return 0
