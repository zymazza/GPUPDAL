...
void PointRef::setFieldInternal(Dimension::Id dim, void *val)
{
    ...
    // D0263: a view-bound coordinate write must move the view's coordinate
    // epoch so a reused cached KD3 product refreshes its snapshot.
    if (m_view)
        m_view->noteCoordinateWrite(dim);
